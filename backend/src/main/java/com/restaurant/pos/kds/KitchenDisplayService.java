...
public class KitchenDisplayService {
    ...
    @KafkaListener(topics = "order-events", groupId = "kds-consumer-group")
    public void handleOrderFiredEvent(OrderFiredEvent event) {
        ...
        // Process items for each Kitchen Section
    ...
    public void displayOrderForSection(String orderNumber, KitchenSection section, List<OrderItemEvent> items) {
        log.info("");
        log.info(">>> {} STATION <<<", section);
        log.info("Order: {}", orderNumber);

        for (OrderItemEvent item : items) {
            String instructions = item.getSpecialInstructions() != null ? "(" + item.getSpecialInstructions() + ")" : "";
